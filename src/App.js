...
const buildPrompt = (mode, matiere, chapitre) => {
  ...
  if (mode === 'Quiz') return `${base}\nGénère un quiz de 4 questions sur "${chapitre}" en ${matiere}.\nPour chaque question :\nQ1: [question]\nA) B) C) D)\n✅ Bonne réponse : [lettre] — [explication]`;
...
// ── STYLES GLOBAUX ──────────────────────────────────────────
const S = {
  app: { maxWidth: 430, margin: '0 auto', minHeight: '100vh', background: '#080812', color: '#F0F0FF', fontFamily: "'Outfit', sans-serif", position: 'relative', overflow: 'hidden' },
...
const Logo = ({ size = 32 }) => (
  <span style={{ fontFamily: "'Unbounded', sans-serif", fontWeight: 900, fontSize: size }}>
    <span style={{ color: '#00F5A0' }}>P</span>
    <span style={{ color: '#F0F0FF' }}>ass</span>
    <span style={{ color: '#00F5A0' }}>I</span>
    <span style={{ color: '#F0F0FF' }}>t</span>
  </span>
);
...
const AuthScreen = ({ onAuth }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!isLogin) {
        // Inscription : créer/vérifier l'utilisateur dans Supabase
  ...
  if (sent) return (
    <div style={S.app}>
      <div style={{ ...S.screen, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 12, color: '#64748B', fontSize: 15, lineHeight: 1.6 }}>On t'a envoyé un lien magique à <strong style={{ color: '#F0F0FF' }}>{email}</strong>. Clique dessus pour accéder à l'app.</div>
        <div style={{ marginTop: 16, fontSize: 13, color: '#475569' }}>Pas reçu ? Vérifie tes spams.</div>
  ...
  return (
    <div style={S.app}>
      <div style={{ ...S.screen, display: 'flex', flexDirection: 'column', padding: '0 24px' }}>
        {/* Hero */}
          ...
          {!isLogin && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 12, color: '#64748B', lineHeight: 1.5 }}>Si mineur(e) : j'ai l'accord de mes parents pour créer ce compte</span>
              </label>
            </div>
          )}
        ...
        <div style={{ textAlign: 'center', marginTop: 32, color: '#334155', fontSize: 12, lineHeight: 1.6 }}>
  ...
  return (
    <div style={S.app}>
      <div style={{ ...S.screen, padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ marginTop: 8, color: '#64748B', textAlign: 'center', fontSize: 14 }}>Choisis une formule pour continuer à réviser</div>

        <div style={{ width: '100%', marginTop: 32, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            { key: 'brevet', label: '🎓 Brevet', desc: "Jusqu'au 30 juin", price: '9,99€', badge: 'Le plus populaire' },
            { key: 'vacances', label: '☀️ Vacances', desc: 'Juillet - Août', price: '4,99€', badge: null },
            { key: 'annuel', label: '📅 Annuel', desc: 'Toutes les classes', price: '29,99€/an', badge: 'Meilleure valeur' },
          ].map(f => (
            <div key={f.key} style={{ ...S.card(), position: 'relative', overflow: 'hidden' }}>
              {f.badge && <div style={{ position: 'absolute', top: 12, right: 12, background: '#00F5A020', color: '#00F5A0', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 20 }}>{f.badge}</div>}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 18, fontWeight: 900, color: '#00F5A0' }}>{f.price}</div>
              </div>
              <a href={`${STRIPE_LINKS[f.key]}?prefilled_email=${encodeURIComponent(user?.email || '')}`} target="_blank" rel="noreferrer" style={{ display: 'block', textAlign: 'center', background: '#00F5A0', color: '#080812', borderRadius: 10, padding: '12px', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
        ...
        <div style={{ marginTop: 24, color: '#334155', fontSize: 12, textAlign: 'center', lineHeight: 1.6 }}>
          🔒 Paiement sécurisé par Stripe · Sans renouvellement automatique (sauf formule annuelle)
        </div>
...
const HomeScreen = ({ user, matieres, onSelectMatiere }) => {
  ...
  return (
    <div style={{ padding: '0 0 16px' }}>
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #0D1830 0%, #080812 100%)', padding: '24px 20px 28px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -60, right: -60, width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, #7B2FFF20, transparent)' }} />
      ...
      <div style={{ padding: '20px 16px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {matieres.map(m => (
            <button key={m.id} onClick={() => onSelectMatiere(m)} style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#12121F', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', width: '100%' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#F0F0FF', fontFamily: "'Unbounded', sans-serif", fontSize: 13 }}>{m.nom}</div>
  ...
  <div>
    ...
    <div style={{ padding: '20px 16px' }}>
      ...
      {[
        ...
        { mode: 'Quiz', emoji: '❓', desc: '4 questions pour tester tes connaissances', color: '#EC4899' },
        ...
        <button key={mode} onClick={() => onSelect(mode)} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, width: '100%', background: '#12121F', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '16px', marginBottom: 10, cursor: 'pointer', textAlign: 'left' }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#F0F0FF', fontFamily: "'Unbounded', sans-serif", fontSize: 13 }}>{mode}</div>
...
const ContentScreen = ({ matiere, chapitre, mode, onBack }) => {
  ...
  const generateContent = async () => {
    ...
    if (cached?.contenu_ia) {
      setContent(cached.contenu_ia);
      setLoading(false);
      await supabase.from('cache_ia').update({ nb_reutilisations: supabase.rpc('increment') }).eq('cle_cache', cacheKey);
      return;
    }
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        {loading && !content && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <div style={{ color: '#64748B', fontSize: 14 }}>L'IA prépare ton contenu…</div>
          </div>
        )}
        {error && <div style={{ background: '#FEF2F2', borderRadius: 12, padding: '14px', color: '#991B1B', fontSize: 14 }}>{error}</div>}
        {content && (
          <div style={{ whiteSpace: 'pre-wrap', fontSize: 14, lineHeight: 1.8, color: '#E2E8F0' }}>
            {content}
            {loading && <span style={{ display: 'inline-block', width: 2, height: 16, background: matiere.couleur_hex, marginLeft: 2, animation: 'blink 1s step-end infinite', verticalAlign: 'middle' }} />}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ padding: '24px 16px' }}>
      <div style={{ marginBottom: 24 }}>
        <Logo size={22} />
        <div style={{ fontSize: 12, color: '#475569', marginTop: 4 }}>Mon profil</div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={S.card()}>
          ...
          {[
            ...
            { label: '📅 Inscrit le', value: user?.date_inscription ? new Date(user.date_inscription).toLocaleDateString('fr-FR') : '—' },
          ].map(({ label, value }) => (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: statusColors[user?.statut] || '#64748B', background: (statusColors[user?.statut] || '#64748B') + '20', padding: '4px 12px', borderRadius: 20 }}>{statusLabels[user?.statut] || '—'}</span>
          </div>
        </div>
        ...
        <div style={S.card()}>
          <div style={{ fontSize: 12, color: '#475569', marginBottom: 14, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Légal & Confidentialité</div>
...
export default function App() {
  ...
  const [dbUser, setDbUser] = useState(null);           // données users table
  ...
  const handleLogout = async () => {
    ...
    setAuthUser(null); setDbUser(null);
  };
  ...
  if (loading) return (
    <div style={{ ...S.app, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <Logo size={32} />
        <div style={{ marginTop: 20, color: '#334155', fontSize: 14 }}>Chargement…</div>
      </div>
    </div>
  );

  if (!authUser) return <AuthScreen onAuth={() => {}} />;
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.screen}>
        ...
        {!selectedMatiere && tab === 'home' && (
          <HomeScreen user={dbUser} matieres={matieres} onSelectMatiere={m => { setSelectedMatiere(m); setSelectedChapitre(null); setSelectedMode(null); }} />
        )}
        {selectedMatiere && !selectedChapitre && (
          <ChapitresScreen matiere={selectedMatiere} chapitres={filteredChapitres} onSelect={ch => setSelectedChapitre(ch)} onBack={() => setSelectedMatiere(null)} />
        )}
        {selectedMatiere && selectedChapitre && !selectedMode && (
          <ModeScreen matiere={selectedMatiere} chapitre={selectedChapitre} onSelect={m => setSelectedMode(m)} onBack={() => setSelectedChapitre(null)} />
        )}
        {selectedMatiere && selectedChapitre && selectedMode && (
          <ContentScreen matiere={selectedMatiere} chapitre={selectedChapitre} mode={selectedMode} onBack={() => setSelectedMode(null)} />
        )}
        {tab === 'profil' && !selectedMatiere && (
          <ProfilScreen user={dbUser} onLogout={handleLogout} />
        )}
      </div>
